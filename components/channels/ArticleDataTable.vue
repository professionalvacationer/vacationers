              ...
              <v-container class="py-0">
                <v-row>
                  <v-img
                    ...
                    max-width="350px"
                    height="150px"
                    :src="require(`~/assets/home/${item.articlePhoto}`)"
                  />
...
export default {
  ...
  methods: {
    setArticle(article) {
      this.$store.dispatch('articles/getCurrentArticle', article)
      this.$emit('is-article', 0)
    },
    filter(value, search, item) {
      const inFirstName = RegExp(search, 'i').test(item.authorFirstName)
      const inLastName = RegExp(search, 'i').test(item.authorLastName)
